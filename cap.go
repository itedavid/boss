...
// cap.go 是「点击点采集」的界面流程层：开始/停止/清空某一组的采集，以及把已采集的点
// 显示到对应的状态标签和列表里。
//
// 底层的活儿在别处：钩子安装/卸载在 hook.go，点在页面上的落点在 hook.go 的队列里，
// 组与组之间的独立关系在 quick.go（它定义了「全局组号」这套约定）。
...
// g 是全局组号（见 quick.go）：1/2=打招呼页两组，>=3=快捷回复页各组。
